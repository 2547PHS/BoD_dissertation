...
mean_dot <- stat_summary(fun = mean, geom = "point", shape = 20, size = 1, 
                         color = "red", fill = "red") 
...
burden$INCOME_GROUP <- factor(burden$INCOME_GROUP, c("<$27,000", "$27,000 to <$52,000",
                                                     "$52,000 to <$85,000",
...
burden$UrbanRural <- factor(burden$UrbanRural, c("Urban", "Rural", "NA"))
...
Table_IncidentCases_income <- burden %>%
  filter(TOTAL_ADULTS > 0) %>%
  group_by(INCOME_GROUP) %>%
  summarise(IncidentCases = sum(EXPECTED_ALLCAUSE_CASES, na.rm = T)) %>%
  mutate_if(is.numeric, round, digits = 0) %>%
...
#Living location
Table_IncidentCases_LivLoc <- burden %>% 
  filter(TOTAL_ADULTS > 0) %>%
  group_by(UrbanRural) %>%
  summarise(IncidentCases = sum(EXPECTED_ALLCAUSE_CASES, na.rm = T)) %>% 
  mutate_if(is.numeric, round, digits = 0)
...
#By median household income
Table_AttributableCases_income <- burden %>%
  filter(TOTAL_ADULTS > 0) %>%
  group_by(INCOME_GROUP) %>%
  summarise(AttributableCases = sum(AC, na.rm = T),
            LowerAttributableCases = sum(AC_LOWER_CI, na.rm = T),
...
#By living location
Table_AttributableCases_LivLoc <- burden %>% 
  filter(TOTAL_ADULTS > 0) %>%
  group_by(UrbanRural) %>%  
  summarise(AttributableCases = sum(AC, na.rm = T), 
            LowerAttributableCases = sum(AC_LOWER_CI, na.rm = T),
...
#By median household income
Table_AdultsTotal_income <- burden %>%
  group_by(INCOME_GROUP) %>%
...
#By living location
Table_AdultsTotal_LivLoc <- burden %>% 
  group_by(UrbanRural) %>%  
  summarise(TOTAL_ADULT_POP = sum(TOTAL_ADULTS, na.rm = T)) %>%
  mutate_if(is.numeric, round, digits = 0)
...
#By median household income
Table_CensusTract_income <- burden %>%
  filter(TOTAL_ADULTS > 0) %>%
  group_by(INCOME_GROUP) %>%
  summarise(CENSUS_TRACTS = length(NAME.x)) %>%
  mutate_if(is.numeric, round, digits = 0)  %>%
...
#By living location
Table_CensusTract_LivLoc <- burden %>% 
  group_by(UrbanRural) %>%  
  summarise(CENSUS_TRACTS = length(NAME.x)) %>% 
  mutate_if(is.numeric, round, digits = 0)
...
burden_WHO_ALL <- burden %>%
  select(YEAR, TOTAL_ADULTS, NO2_LEVELS, EXPECTED_ALLCAUSE_CASES, ERF, UNIT) %>% 
...
burden_counterfactual_ALL <- burden_counterfactual_ALL[,-c(3:5)] #get rid of unnecessary columns

#Rename columns
colnames(burden_counterfactual_ALL) <- c("TOTAL_ADULT_POP", "AttributableCases",
...
burden_counterfactual_ALL <- burden_counterfactual_ALL %>%
  mutate(AttributableCases_difference = AttributableCases - AttributableCases_counterfactual)

#Rename columns
colnames(burden_counterfactual_ALL) <- c("TOTAL_ADULT_POP", "AttributableCases_baseline",
                                         "AttributableCases_counterfactual",
                                         "AttributableCases_difference")
#View table
view(burden_counterfactual_ALL)
...
burden_WHO_white <- burden %>%
  select(YEAR, WHITE_ALONE, NO2_LEVELS, EXPECTED_WHITE, ERF, UNIT) %>% 
  mutate(NO2_LEVELS = ifelse(NO2_LEVELS > 10, 10, NO2_LEVELS),
         RRnew = exp((log(ERF)/UNIT) * NO2_LEVELS),
         PIF = (RRnew - 1)/(RRnew), 
...
#Rename columns
colnames(burden_WHO_white) <- c("TOTAL_ADULT_POP", "AttributableCases_counterfactual")
...
burden_WHO_black <- burden %>%
  select(YEAR, BLACK_ALONE, NO2_LEVELS, EXPECTED_BLACK, ERF, UNIT) %>% 
  mutate(NO2_LEVELS = ifelse(NO2_LEVELS > 10, 10, NO2_LEVELS),
         RRnew = exp((log(ERF)/UNIT) * NO2_LEVELS),
         PIF = (RRnew - 1)/(RRnew), 
...
#Rename columns
colnames(burden_WHO_black) <- c("TOTAL_ADULT_POP", "AttributableCases_counterfactual")
...
burden_WHO_AIAN <- burden %>%
  select(YEAR, AIAN_ALONE, NO2_LEVELS, EXPECTED_AIAN, ERF, UNIT) %>% 
  mutate(NO2_LEVELS = ifelse(NO2_LEVELS > 10, 10, NO2_LEVELS),
         RRnew = exp((log(ERF)/UNIT) * NO2_LEVELS),
         PIF = (RRnew - 1)/(RRnew), 
...
#Rename columns
colnames(burden_WHO_AIAN) <- c("TOTAL_ADULT_POP", "AttributableCases_counterfactual")
...
burden_WHO_asian <- burden %>%
  select(YEAR, ASIAN_ALONE, NO2_LEVELS, EXPECTED_ASIAN, ERF, UNIT) %>% 
  mutate(NO2_LEVELS = ifelse(NO2_LEVELS > 10, 10, NO2_LEVELS),
         RRnew = exp((log(ERF)/UNIT) * NO2_LEVELS),
         PIF = (RRnew - 1)/(RRnew), 
...
#Rename columns
colnames(burden_WHO_asian) <- c("TOTAL_ADULT_POP", "AttributableCases_counterfactual")
...
burden_WHO_NHPI <- burden %>%
  select(YEAR, NHPI_ALONE, NO2_LEVELS, EXPECTED_NHPI, ERF, UNIT) %>% 
  mutate(NO2_LEVELS = ifelse(NO2_LEVELS > 10, 10, NO2_LEVELS),
         RRnew = exp((log(ERF)/UNIT) * NO2_LEVELS),
         PIF = (RRnew - 1)/(RRnew), 
...
#Rename columns
colnames(burden_WHO_NHPI) <- c("TOTAL_ADULT_POP", "AttributableCases_counterfactual")
...
burden_WHO_other <- burden %>%
  select(YEAR, OTHER_ALONE, NO2_LEVELS, EXPECTED_OTHER, ERF, UNIT) %>% 
  mutate(NO2_LEVELS = ifelse(NO2_LEVELS > 10, 10, NO2_LEVELS),
         RRnew = exp((log(ERF)/UNIT) * NO2_LEVELS),
         PIF = (RRnew - 1)/(RRnew), 
...
#Rename columns
colnames(burden_WHO_other) <- c("TOTAL_ADULT_POP", "AttributableCases_counterfactual")
...
burden_WHO_twomore <- burden %>%
  select(YEAR, TWO_MORE, NO2_LEVELS, EXPECTED_TWOMORE, ERF, UNIT) %>% 
  mutate(NO2_LEVELS = ifelse(NO2_LEVELS > 10, 10, NO2_LEVELS),
         RRnew = exp((log(ERF)/UNIT) * NO2_LEVELS),
         PIF = (RRnew - 1)/(RRnew), 
...
#Rename columns
colnames(burden_WHO_twomore) <- c("TOTAL_ADULT_POP", "AttributableCases_counterfactual")
...
burden_WHO_HisLat <- burden %>%
  select(YEAR, HisLat_TOTAL, NO2_LEVELS, EXPECTED_HisLat, ERF, UNIT) %>% 
  mutate(NO2_LEVELS = ifelse(NO2_LEVELS > 10, 10, NO2_LEVELS),
         RRnew = exp((log(ERF)/UNIT) * NO2_LEVELS),
         PIF = (RRnew - 1)/(RRnew), 
...
#Rename columns
colnames(burden_WHO_HisLat) <- c("TOTAL_ADULT_POP", "AttributableCases_counterfactual")
...
burden_WHO_NotHisLat <- burden %>%
  select(YEAR, NotHisLat_TOTAL, NO2_LEVELS, EXPECTED_NotHisLat, ERF, UNIT) %>% 
  mutate(NO2_LEVELS = ifelse(NO2_LEVELS > 10, 10, NO2_LEVELS),
         RRnew = exp((log(ERF)/UNIT) * NO2_LEVELS),
         PIF = (RRnew - 1)/(RRnew), 
...
#Rename columns
colnames(burden_WHO_NotHisLat) <- c("TOTAL_ADULT_POP", "AttributableCases_counterfactual")
...
burden_counterfactual_race <- burden_counterfactual_race[,-c(3:5)] #get rid of unnecessary columns
...
burden_WHO_income <- burden %>%
  select(YEAR, INCOME_GROUP, NO2_LEVELS, EXPECTED_ALLCAUSE_CASES, ERF, UNIT) %>% 
  mutate(NO2_LEVELS = ifelse(NO2_LEVELS > 10, 10, NO2_LEVELS),
         RRnew = exp((log(ERF)/UNIT) * NO2_LEVELS),
         PIF = (RRnew - 1)/(RRnew), 
         AC = PIF * EXPECTED_ALLCAUSE_CASES) %>%
  select(-ERF, -UNIT, -RRnew, -UNIT) %>%
  group_by(INCOME_GROUP) %>%
  summarise(AttributableCases = sum(AC, na.rm = T)) %>%
  mutate_if(is.numeric, round, digits = 0) %>%
  arrange(factor(INCOME_GROUP, levels = c("<$27,000", "$27,000 to <$52,000",
                                          ...
                                          "$85,000 to <$141,000", ">=$141,000")))
#Change column name
colnames(burden_WHO_income) <- c("TOTAL_ADULT_POP", "AttributableCases_counterfactual")
...
burden_counterfactual_income <- burden_counterfactual_income[,-c(3:5)] #get rid of unnecessary columns
...
burden_WHO_LivLoc <- burden %>%
  select(YEAR, UrbanRural, NO2_LEVELS, EXPECTED_ALLCAUSE_CASES, ERF, UNIT) %>% 
  mutate(NO2_LEVELS = ifelse(NO2_LEVELS > 10, 10, NO2_LEVELS),
         RRnew = exp((log(ERF)/UNIT) * NO2_LEVELS),
         PIF = (RRnew - 1)/(RRnew), 
         AC = PIF * EXPECTED_ALLCAUSE_CASES) %>%
...
#Change column name
colnames(burden_WHO_LivLoc) <- c("TOTAL_ADULT_POP", "AttributableCases_counterfactual")
...
burden_counterfactual_LivLoc <- burden_counterfactual_LivLoc[,-c(3:5)] #get rid of unnecessary columns
...
burden_WHO_state <- burden %>%
  select(YEAR, STATE.x, NO2_LEVELS, EXPECTED_ALLCAUSE_CASES, ERF, UNIT) %>% 
  mutate(NO2_LEVELS = ifelse(NO2_LEVELS > 10, 10, NO2_LEVELS),
         RRnew = exp((log(ERF)/UNIT) * NO2_LEVELS),
         PIF = (RRnew - 1)/(RRnew), 
         AC = PIF * EXPECTED_ALLCAUSE_CASES) %>%
...
burden_counterfactual_state <- burden_counterfactual_state[,-c(3:5)] #get rid of duplicated column
...
#Rename columns
colnames(burden_counterfactual_state) <- c("STATE", "AttributableCases_baseline",
                                           "AttributableCases_counterfactual",
                                           "AttributableCases_difference")
#View table
