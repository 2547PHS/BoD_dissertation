...
#Create object for national adult-onset asthma incidence rate and its lower and upper limits
IR  <- 0.0038
IR_LOWER <- 0.0032 #lower confidence interval (CI)
IR_UPPER <- 0.0044 #upper CI

#Perform calculations
burden <- census2020_combined %>% 
  mutate(PRV = 0.084, #prevalence rate
         PRV_LOWER = 0.0819, #prevalence rate lower CI
         PRV_UPPER = 0.0861, #prevalence rate upper CI
         ERF = 1.11, #exposure response function (ERF)
         ERF_LOWER_CI = 1.05, #ERF lower CI
         ...
         EXPECTED_ALLCAUSE_CASES_LOWER_CI =
           (TOTAL_ADULTS - (TOTAL_ADULTS * PRV_LOWER)) * IR_LOWER, #expected all-cause cases lower CI
         EXPECTED_ALLCAUSE_CASES_UPPER_CI =
           (TOTAL_ADULTS - (TOTAL_ADULTS * PRV_UPPER)) * IR_UPPER, #expected all-cause cases upper CI
         ...
         AC_LOWER_CI = PIF_LOWER_CI * EXPECTED_ALLCAUSE_CASES_LOWER_CI, #AC lower CI
         AC_UPPER_CI = PIF_UPPER_CI * EXPECTED_ALLCAUSE_CASES_UPPER_CI) #AC upper CI
...
#First, consider number of census tracts with NO2 levels > WHO threshold

above_WHO_level <- burden %>% 
  filter(NO2_LEVELS > 10) %>%
  group_by(sum(TOTAL_ADULTS)) %>% 
  summarise(Above_WHO_level = length(NO2_LEVELS)) %>% 
  mutate_if(is.numeric, round, digits = 2) %>%
  as.data.frame()

#View answer
view(above_WHO_level)


#U.S. adult pop counterfactual calculation
